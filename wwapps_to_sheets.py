...
def create_wwapps_sheet(service):
    """Создает новый лист WWApps если его нет"""
    try:
        # Получаем информацию о таблице
        spreadsheet = service.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
        sheets = spreadsheet.get('sheets', [])
        
        # Проверяем есть ли уже лист WWApps
        wwapps_sheet_exists = False
        for sheet in sheets:
            if sheet['properties']['title'] == 'WWApps':
                wwapps_sheet_exists = True
                print("📋 Лист WWApps уже существует")
                break
        
        if not wwapps_sheet_exists:
            # Создаем новый лист
            print("📋 Создаю новый лист WWApps...")
            request = {
                'addSheet': {
                    'properties': {
                        'title': 'WWApps',
...
def parse_message_data(text):
    """Парсит данные из сообщения канала"""
    data = {}
...
async def get_wwapps_from_channel():
    """Получает все приложения WWApps из канала"""
    await client.start()
    ...
    try:
        ...
        async for message in client.iter_messages(channel):
            if message.text and ('🤖' in message.text and 'Бот:' in message.text) and ('WildWildApps' in message.text or 'wwapps_bot' in message.text):
        ...
        print(f"📱 Найдено уникальных приложений WWApps: {len(apps)}")
...
def prepare_sheets_data(apps):
    ...
    headers = [
        ...
        'Срок жизни (дни)', 'Статус', 'URL', 'ID сообщения', 'Категория'
    ]
    rows.append(headers)
    ...
    for app in apps:
        ...
        row = [
            ...
            app.get('category', '')                        # J: Категория
        ]
        rows.append(row)
    
    return rows

def update_wwapps_sheet(service, data):
    """Обновляет данные в листе WWApps"""
    try:
        # Очищаем лист WWApps
        print("🧹 Очищаю старые данные в листе WWApps...")
        ...
        print(f"📝 Записываю {len(data)} строк в лист WWApps...")
        body = {
            'values': data,
            'majorDimension': 'ROWS'
        }
        ...
        print(f"✅ Обновлено {result.get('updatedRows', 0)} строк в листе WWApps")
        ...
        format_requests = [
            # Автоматическая ширина колонок
            {
                "autoResizeDimensions": {
                    "dimensions": {
                        "sheetId": get_sheet_id(service, 'WWApps'),
                        "dimension": "COLUMNS",
                        "startIndex": 0,
                        "endIndex": 10
                    }
        ...
        print("✅ Лист WWApps отформатирован")
        
    except Exception as e:
        print(f"❌ Ошибка при обновлении листа WWApps: {e}")
...
async def main():
    try:
        # Получаем данные WWApps из канала
        apps = await get_wwapps_from_channel()
        
        # Статистика
        print("\n📊 Статистика WWApps:")
        bundle_count = sum(1 for app in apps if app.get('bundle_id'))
        banned_count = sum(1 for app in apps if app['status'] == 'Забанено')
        ...
        # Создаем лист WWApps если нужно
        if not create_wwapps_sheet(service):
            return
        
        # Подготавливаем данные
        sheets_data = prepare_sheets_data(apps)
        
        # Обновляем лист WWApps
        update_wwapps_sheet(service, sheets_data)
        
        print("\n✅ Готово! Проверьте лист WWApps в вашей таблице")
...
if __name__ == "__main__":
    print("📊 Синхронизация WWApps с Google Sheets")
    print("=" * 50)
    asyncio.run(main()) 